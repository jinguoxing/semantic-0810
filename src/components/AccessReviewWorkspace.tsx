...
export const AccessReviewWorkspace: React.FC<AccessReviewWorkspaceProps> = ({ onNavigateToAiHome, onNavigateToAuthorizationRecords, onNavigateToPolicyManagement, onNavigateToAuditLogs, onOpenDetail, addToast }) => {
  ...
  return <AdminCenterShell activeItem={activeItem} onNavigate={navigate}><main className="flex-1 overflow-y-auto"><div className="mx-auto max-w-6xl space-y-5 p-6 lg:p-8">
    ...
    <section className="space-y-3">{items.map((item) => <article key={item.id} className="rounded-lg border border-[#E6EAF0] bg-white p-4 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0 space-y-2"><div className="flex flex-wrap items-center gap-2"><h2 className="font-bold">{item.resourceName}</h2><span className="rounded border border-[#E2E8F0] bg-[#F8FAFC] px-2 py-0.5 text-[11px] text-[#475569]">{item.requestedAction}</span><span className={`rounded border px-2 py-0.5 text-[11px] font-bold ${riskClass[item.riskLevel]}`}>{item.riskLevel === 'HIGH' ? '高风险' : item.riskLevel === 'MEDIUM' ? '中等风险' : '低风险'}</span></div><p className="text-xs text-[#475569]">{item.resourceActionSummary}</p><div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-[#667085]"><span>{item.applicantName} · {item.applicantDepartment}</span><span>{item.parentSubmissionTitle}</span><span>{item.submittedAtText}</span><span className="font-mono">{item.requestId}</span></div><div className="rounded bg-[#F8FAFC] px-3 py-2 text-xs text-[#475569]"><strong className="text-[#334155]">触发原因：</strong>{item.reviewReasonDescription}</div></div><div className="flex shrink-0 flex-wrap gap-2"><button onClick={() => onOpenDetail?.(item.requestId)} className="inline-flex items-center gap-1 rounded border border-[#CBD5E1] px-3 py-2 text-xs font-bold text-[#334155]">查看决策 <ChevronRight className="h-3.5 w-3.5" /></button><button onClick={() => decide(item, 'DENY')} className="rounded border border-[#FECDCA] px-3 py-2 text-xs font-bold text-[#B91C1C]">不授权</button><button onClick={() => decide(item, 'GRANT_WITH_LIMITS')} className="inline-flex items-center gap-1 rounded bg-[#2563EB] px-3 py-2 text-xs font-bold text-white"><Check className="h-3.5 w-3.5" />按建议授权</button></div></div></article>)}{items.length === 0 && <div className="rounded-lg border border-dashed border-[#CBD5E1] p-10 text-center text-sm text-[#667085]"><ShieldCheck className="mx-auto mb-2 h-5 w-5" />当前没有待处理的访问申请。</div>}</section>

...
export const RightWorkspaceAskPlan: React.FC<RightWorkspaceAskPlanProps> = ({
  ...
  const PlanBody = () => (
    <div className="space-y-5">
      ...
      <section className="space-y-2"><div className="flex items-center space-x-1.5 border-b border-[#F1F5F9] pb-1 text-xs font-bold text-[#0F172A]"><span className="h-3.5 w-1.5 rounded-full bg-[#2563EB]" /><span>拟使用输入（{coreResources.length} 项）</span></div><div className="grid grid-cols-2 gap-2">{coreResources.map((resource) => <div key={resource.id} className="rounded-lg border border-[#E2E8F0] bg-white p-2.5"><div className="font-bold text-[#0F172A]">{resource.name}</div><div className="mt-0.5 text-[11px] text-[#64748B]">来自当前数据方案 · {resource.type} · {resource.granularity}</div></div>)}</div></section>

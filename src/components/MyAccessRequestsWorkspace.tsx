...
export const MyAccessRequestsWorkspace: React.FC<MyAccessRequestsWorkspaceProps> = ({ onNavigateToDiscovery, onNavigateToResources, onNavigateToSolution, onResumeAnalysisTask, addToast }) => {
  ...
  const resume = (item: MyAccessSubmissionViewModel) => {
    ...
    addToast?.(mode === 'degraded' ? 'info' : 'success', mode === 'degraded' ? '按当前授权恢复任务' : '恢复分析任务', '已恢复原分析任务及已授权的数据方案上下文。');
  };
  ...
  return <div className="flex-1 overflow-y-auto bg-[#F8FAFC] text-[#172033]"><main className="mx-auto w-full max-w-[1320px] space-y-6 px-8 py-7">
    ...
    <div className="grid gap-3 rounded-lg border border-[#E6EAF0] bg-[#F8FAFC] p-3 md:grid-cols-[1fr_auto_auto_auto]"><label className="relative"><Search className="absolute left-3 top-2.5 h-3.5 w-3.5 text-[#94A3B8]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索任务、用途或资源" className="w-full rounded border border-[#E2E8F0] bg-white py-2 pl-8 pr-3 text-xs" /></label><select value={source} onChange={(event) => setSource(event.target.value)} className="rounded border border-[#E2E8F0] bg-white px-3 text-xs">{sources.map((value) => <option key={value} value={value}>{value === 'all' ? '全部来源' : value}</option>)}</select><select value={period} onChange={(event) => setPeriod(event.target.value as typeof period)} className="rounded border border-[#E2E8F0] bg-white px-3 text-xs"><option value="all">全部时间</option><option value="today">今天</option><option value="7days">近 7 天</option><option value="30days">近 30 天</option></select><select value={sort} onChange={(event) => setSort(event.target.value as typeof sort)} className="rounded border border-[#E2E8F0] bg-white px-3 text-xs"><option value="recent">最近更新</option><option value="submitted">提交时间</option></select></div>
    <section className="space-y-4">{filtered.map((item) => { const style = readiness[item.taskReadiness]; const Icon = style.Icon; const open = Boolean(expanded[item.id]); return <article key={item.id} className="overflow-hidden rounded-lg border border-[#E6EAF0] bg-white shadow-sm"><div className="flex flex-col gap-4 p-5 sm:flex-row sm:justify-between"><div className="min-w-0 space-y-2"><div className="flex flex-wrap items-center gap-2"><h2 className="text-base font-bold">{item.taskTitle}</h2><span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-bold ${style.className}`}><Icon className="h-3.5 w-3.5" />{style.label}</span></div><p className="text-xs text-[#667085]">{item.purposeDescription}</p><div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-[#667085]"><span>{item.source}</span><span>提交：{item.submittedTime}</span><span>更新：{item.updatedTime}</span></div><div className="grid gap-3 rounded bg-[#F8FAFC] px-3 py-2 text-xs sm:grid-cols-[1fr_auto]"><div><span className="text-[#98A2B3]">访问进展</span><div className="mt-0.5 font-semibold text-[#334155]">{item.accessProgress}</div></div><div><span className="text-[#98A2B3]">当前任务</span><div className="mt-0.5 font-bold">{style.label}</div></div></div><p className="text-xs text-[#475569]">{item.taskReadinessNote}</p></div><div className="flex shrink-0 gap-2"><button onClick={() => primaryAction(item)} className={`inline-flex items-center gap-1 rounded px-3 py-2 text-xs font-bold ${item.taskReadiness === 'BLOCKED' ? 'border border-[#CBD5E1] text-[#334155]' : 'bg-[#2563EB] text-white'}`}><Play className="h-3.5 w-3.5" />{item.primaryAction.label}</button><button onClick={() => setExpanded((current) => ({ ...current, [item.id]: !open }))} className="rounded border border-[#E2E8F0] px-3 py-2 text-xs font-semibold">查看进展 {open ? <ChevronUp className="inline h-3.5" /> : <ChevronDown className="inline h-3.5" />}</button></div></div>{open && <div className="space-y-3 border-t border-[#E6EAF0] bg-[#FAFCFF] p-4"><div className="text-xs font-bold">包含的资源访问请求（{item.requests.length}）</div>{item.requests.map((request) => <div key={request.id} className="rounded border border-[#E6EAF0] bg-white p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><div className="text-xs font-bold">{request.name}</div><div className="mt-1 text-[11px] text-[#667085]">{request.typeBadge} · {request.roleBadge} · {request.operation}</div></div><div className="text-right"><span className={`text-xs font-bold ${stateColor[request.accessState]}`}>● {request.stateLabel}</span><button className="ml-3 text-[11px] font-semibold text-[#2563EB]">查看申请</button></div></div><p className="mt-2 text-xs text-[#475569]">{request.scopeSummary}</p><p className="mt-1 text-[11px] text-[#667085]">有效期：{request.validity}</p>{request.processingNote && <p className="mt-1 text-[11px] text-[#B45309]">{request.processingNote}</p>}{request.deniedNote && <p className="mt-1 text-[11px] text-[#B91C1C]">{request.deniedNote}</p>}</div>)}{item.taskReadiness === 'DEGRADED' && item.limitations && <div className="rounded border border-[#FDE68A] bg-[#FFFBEB] p-3 text-xs"><div><strong>当前授权仍可支持：</strong>{item.limitations.canDo.join(' · ')}</div><div className="mt-1"><strong>当前限制：</strong>{item.limitations.restricted.join(' · ')}</div></div>}</div>}</article>; })}{filtered.length === 0 && <div className="rounded-lg border border-dashed border-[#CBD5E1] p-10 text-center text-sm text-[#667085]"><Filter className="mx-auto mb-2 h-5 w-5" />没有符合当前筛选条件的申请。</div>}</section>
